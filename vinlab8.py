...
def lexer(text):
    ...
    while i < len(text):
        ...
        if match:
            value = match.group(0)
            tokens.append((TOKEN_INT, value))
            i += len(value)
            continue

        # Неизвестный токен
        errors.append((TOKEN_INVALID, text[i]))
        i += 1
...
class Parser:
    def __init__(self, tokens, output_table, file_path):
        self.tokens = tokens
        self.pos = 0
    ...
    def log(self, message):
        self.output_table.insert("", "end", values=(
            f"S{self.step_counter:03}", "Вызов", message, str(self.step_counter), self.file_path, str(self.line_number)
        ))
        self.step_counter += 1

    def error(self, expected, actual):
        self.output_table.insert("", "end", values=(
            "E", "Ошибка", f"Ожидалось '{expected}', встретилось '{actual}'", str(self.step_counter), self.file_path, str(self.line_number)
        ))
        self.step_counter += 1
    ...
    def variable_declaration(self):
        self.log("variable-declaration")

        if self.current()[0] == TOKEN_VAR:
            self.advance()
        else:
            self.error('var', self.current()[1])
            return  # Возврат на уровень выше

        self.integer_constant()

        while self.current()[0] == TOKEN_SEMICOLON:
            self.advance()
            if self.current()[0] == TOKEN_VAR:
                self.variable_declaration()
            else:
                self.error('var', self.current()[1])
                break  # Ошибка, но двигаемся дальше

    def integer_constant(self):
        self.log("integer-constant")

        if self.current()[0] == TOKEN_INT:
            self.advance()
        else:
            self.error('целое число', self.current()[1])

# --- Обработчик анализа ---
def analyze_variable_declaration(text, file_path):
    tokens, lex_errors = lexer(text)

    for row in output_table.get_children():
        output_table.delete(row)

    for i, (code, val) in enumerate(lex_errors, start=1):
        output_table.insert("", "end", values=(
            f"E{i:03}", "Лексическая ошибка", f"Недопустимый символ '{val}'", str(i), file_path, "1"
        ))

    parser = Parser(tokens, output_table, file_path)
    parser.parse()

    if not lex_errors and all(row[0] != "E" for row in output_table.get_children()):
...
def syntax_analysis():
    ...
    if current_tab and hasattr(current_tab, "text_area"):
        ...
        analyze_variable_declaration(text, file_path)
    else:
